...
// Uses BigInt since the result is wrong with regular doubles
function canShiftIntermediate(value) {
  for (let i = 0n; i < 32n; i += 2n) {
    let andval = (0xFFFFFF00n >> i) | (0xFFFFFF00n << (-i & 31n));

    if ((BigInt(value) & andval) == 0n) {
      return true;
    }
  }
  return false;
