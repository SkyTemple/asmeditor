...
import { getLabelForVariableIndex, MAX_UINT32_VALUE } from './nodes/shared.js';
...
export class CodeModel {
  ...
  generateCode() {
    ...
    let validationError = this.generateCodeForNodeList(this.head, context)
      || this.variables.some(variable => variable.validationError);
    
    if (validationError) {
      return "Couldn't generate code due to validation errors.\nPlease fix all blocks and variables highlighted in red.";
    }
  ...
  notifyVariableUpdated(variable) {
    variable.validationError = undefined;
    if (variable.type === 'int') {
      if (isNaN(parseInt(Number(variable.defaultValue)))) {
        variable.validationError = 'Default value must be an integer';
      } else if (variable.defaultValue < 0 || variable.defaultValue > MAX_UINT32_VALUE) {
        variable.validationError = `Default value must be less than or equal to ${MAX_UINT32_VALUE}`;
      }
    }
